...
class SellUsedPhone extends StatefulWidget {
  const SellUsedPhone({Key key}) : super(key: key);

  @override
  _SellUsedPhoneState createState() => _SellUsedPhoneState();
}

class _SellUsedPhoneState extends State<SellUsedPhone> {
  double height=0;
  double width=0;
  ...
  usedMobileData(brand,model,ram,rom,warranty,checkWarranty,condition,repaired,ptaApproved,demandPrice,details)async{
    ...
    print(imgurl);
     ...
     await FirebaseFirestore.instance.collection("usedmobiles").doc().set(
        {
          'Brand':brand,
          'Model': model,
          'Ram': ram,
